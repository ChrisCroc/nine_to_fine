...
- **Jeudi 28 mai** :
  ...
  - **Après-midi (Production, en cours en CLI)** : génération des 6 models structurels (Category, Garment, Outfit, OutfitGarment, Tag, Tagging) avec migrations + validations + fixtures + tests Minitest. Workflow : Chris code en VS Code + shell, Claude Code CLI coache en parallèle. État mental : confiant sur Ruby/models, **fébrile sur les tests Minitest** (faiblesse identifiée à travailler).
  - **Points à consolider en exo ce soir** :
    - **Queries** : `.pluck` (pas clair), différence `.includes` vs `.joins` dans le contexte du N+1, `has_many :through` avec option `:source` (à re-pratiquer).
    - **Tailwind** : groups nommés (`group/name` + `group-hover/name:`), variants enfants `*:` et `**:`.
    - **Models/Tests** : à compléter en fin d'aprem selon ce qui aura été identifié pendant la production.
