...
**Recasage slot Rails** : « API Rails + RSpec à fond » démarre **mercredi 1er juil matin** (mardi 30 matin désormais pris par le lancement emploi, voir ci-dessous). Le track Rails tourne ensuite chaque matin (mer-ven), pas de créneau dédié à caser.
...
- **Lundi 29 juin (avant-midi + aprem)** — React, section **« Static pages »** Scrimba (JSX + composants + props), Chris code tous les challenges. Rythme de reprise post-vacances (pauses), ~62 %+ de la section en fin d'aprem. ⭐ **Décision aprem : continuer React** (Chris était lancé) plutôt que basculer sur l'emploi. _(à compléter en fin de journée)_
- ⭐ **Lancement recherche emploi reporté à mardi 30 matin** (README pinned `nine-to-fine` + LinkedIn headline dev-first + 3 candidatures Rails). ⚠️ **Plancher** : déjà glissé d'1 sem (vacances) ; ne doit pas re-glisser au-delà de mardi matin — c'est le seul livrable borné fin sept.
- ⭐⭐ **Décision stratégique (lun 29 soir) — jalons NON négociables fixés par Chris** : (1) **IA suggester** + (2) **public/privé (US18)** intégrés **avant mi-juillet** ; (3) **app déployée mi-juillet** (~sem 29). Constat de Chris : NtF encore « amateur » sans ses 2 fonctions phares (l'IA = différenciateur vitrine annoncé au PLANNING ; le public/privé = sens du social déjà codé likes/comments/follow). **Contredit le planning actuel** (IA en Phase 4 sept, US18 reporté sine die). **Point technique** : l'IA suggester = Rails (service object `app/services/ai/` + API Anthropic + credentials chiffrées), **ne dépend PAS de React** → codable dès maintenant. ⭐ **Brainstorm roadmap produit demain matin EN PREMIER** (avant l'emploi). ⚠️ **Tension à arbitrer au brainstorm** : ce scope sur ~2 sem percute le démarrage React (Phase 2) ET l'emploi — quelque chose doit céder (probablement **React en sourdine ~2 sem** + NtF redevient prioritaire). À trancher demain, puis logger dans `DECISIONS/`.

- ⭐⭐ **Mardi 30 juin (Claude Code CLI) — grosse journée déploiement-prep.** Brainstorm roadmap **EN PREMIER** (pipeline `brainstorming`) → arbitrage tranché + loggé `DECISIONS/2026-06-30-roadmap-deploy-ia-emploi.md` (**PR #143**) : **déploiement = priorité #1** (URL live = levier emploi), emploi #2 (ne cède pas), **IA suggester #3**, **US18 poussé**, **React bridé à un plancher 1-2h/j** (pas arrêté). ⚠️ Pitch corrigé : **ne PAS revendiquer React/TS au CV** (survente). Puis exécution déploiement-first :
  - **Sécurité** : **PR #144** `fix(security)` msgpack 1.8.1 → 1.8.3 (advisory DFVULN-839 use-after-free transitive via bootsnap, bloquait `scan_ruby` — 3e du genre après crass/net-imap).
  - **Gate Bullet pré-déploiement** : **PR #145** — N+1 réel trouvé via `log/bullet.log` (`Comment => [:user]` sur `outfits#show`, **pas** la « régression Follow » supposée), fixé par `includes(comments: :user)` au controller + **verrouillé** en armant Bullet en test (`Bullet.raise` + hooks + safelist répliqué) + request spec de régression. Audit manuel propre. Notes : [[bullet-n+1-guard-in-test-suite]].
  - ⭐ **Infra provisionnée** (en mode coach, Chris exécute) : domaine **`ninetofine.app`** acheté chez **Porkbun** (~10$/an, WHOIS privacy + auto-renew) ; serveur **Hetzner CX23** (x86, 2 vCPU, 4 Go, **Nuremberg**, **6,64 €/mo TTC**, IP **`78.47.104.157`**, Ubuntu 26.04) ; clé SSH `id_ed25519` validée (`root@78.47.104.157`) ; DNS `A` apex + www → IP, **propagé partout**. ⭐ Décision tranchée : **Kamal sur Hetzner** (défaut Rails 8 + meilleur signal entretien + le moins cher), fallback Fly.io. ⚠️ **Arm CAX en rupture EU** → bascule x86 → **build cross-compilation à gérer** (Mac M4 ARM → image amd64).
  - ⭐ **Habillage complet des 5 pages auth Devise** (**PR #146**) : nested layouts (`_head` partagé + `devise.html.erb` + `layout_by_resource`), partial réutilisable `_field` (dispatch `send`, erreurs inline par champ), account settings + redirect profil après update (`Users::RegistrationsController`). Spec + plan committés `docs/superpowers/`. Notes : [[devise-auth-nested-layouts-and-field-partial]], [[devise-controllers-customization-and-redirects]], [[preflight-form-and-list-reset]].
  - **Polish deploy-relevant** (branche `feature/alert-redirect` mergée) : flashs en **toast flottant** auto-dismiss + fermeture (Stimulus `flash_controller`, hors flux → plus de bande blanche) + **redirect connecté → garments** (`authenticated :user` root).
  - ⚠️ **Non fait aujourd'hui** : **lancement recherche emploi** (encore reporté — re-glissement, à confronter), **React** (plancher 1-2h/j non tenu), **exo sandbox soir**.

**⭐⭐ ACCOMPLI (mardi 30 juin PM, Claude Code CLI) — NINE TO FINE EST DÉPLOYÉE EN PRODUCTION 🎉**

App **live sur https://ninetofine.app + https://www.ninetofine.app** (HTTPS valide, redirection http→https, vérifiées par `curl`). **Jalon « v1 déployée » atteint le 30 juin, très en avance** sur la cible roadmap (~fin juillet sem 30). Mode coach strict (Chris a tout tapé), doc Kamal/GitHub vérifiée avant chaque commande (`feedback-verify-external-setup`). **2 PR mergées** : **#149** (config Kamal complète) + **#150** (host `www` au proxy). Notes Obsidian détaillées FR : [[kamal-deploiement-fondamentaux]] + [[database-url-underscore-uri-parser]].

- **Stack déployée** : Kamal 2.11 → Hetzner CX23, image sur **ghcr** (token classic **moindre privilège** `write:packages` seul, piège auto-cochage `repo` contourné via URL `?scopes=`), **PostgreSQL 17 en accessory** sur le même host (volume persistant), **SSL Let's Encrypt auto** via kamal-proxy, build **amd64 depuis Mac ARM** en **QEMU local** (remote builder reporté = make-it-work-first). **Docker Desktop installé** sur le Mac (`brew install --cask docker-desktop`, n'était pas présent).
- ⭐ **Architecture secrets 3 niveaux** : `deploy.yml` = noms → `.kamal/secrets` = commandes `$(cat …)` → fichiers gitignorés = vraies valeurs (rien de secret dans git). Mdp Postgres généré `openssl rand -hex 32`.
- ⭐ **2 bugs résolus en méthode systématique (repro avant fix)** : (1) **1er `kamal setup` crashé** — conteneur jamais *healthy* car crash au **parsing de `DATABASE_URL`** : le host accessory `nine_to_fine-db` a des **underscores**, refusés par le parser `URI::RFC2396_Parser` d'AR (en dev `localhost` masquait). Fix = `database.yml` prod en **paramètres séparés** (host/user/password/database) au lieu d'`url:`, anchors YAML pour les 4 connexions. (2) **`www` non routé** — proxy ne connaissait que l'apex → `host:` → `hosts:` (liste).
- ⚠️ **Le site tourne 24/7 tout seul** (point clé expliqué à Chris) : `kamal deploy` = publier une version, pas allumer le site. Pas de relance manuelle.

**▶️ PROCHAINE TÂCHE (ordre roadmap décalé d'un cran, déploiement fait)** : **assets recherche emploi** (README propre `nine-to-fine` **avec URL live** + nettoyage doc publique qui expose `PLANNING`/`CLAUDE.md` mode coach + LinkedIn dev-first + CV) → candidatures ; puis **IA suggester** (brainstorm produit dédié) ; **durcir le serveur** (firewall Hetzner 22/80/443 + fail2ban, bonne histoire entretien) ; **React** plancher 1-2h/j à réamorcer.

**Reports / backlog** : ⚠️ **lancement emploi = LA priorité maintenant** (re-glissé 2×, déploiement fait → plus d'excuse), skill `job-tracker`, US18 brainstorm produit, US05 delete account, **6 PR Dependabot** ouvertes (#130/131/132/133/141/142 — batch-merge après rebase ; **kamal #133 désormais débloqué**, 1er déploiement passé), backlog auth post-deploy (home landing brainstorm, toggle visibilité mot de passe), **object storage** Active Storage (post-deploy near-term), **durcissement serveur** (firewall + fail2ban), **redirect www→apex** (polish SEO), exos sandbox soir cumulés, **React plancher non tenu** (3e jour). _(détail dans `to-do.md` Obsidian)_
