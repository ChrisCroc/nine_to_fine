...
**⭐ POINT DE DÉPART — session fraîche cet après-midi (mardi 30 juin PM)**

- **État repo** : `main` à jour, working tree propre. Toutes les PR du jour mergées (#143 roadmap, #144 msgpack, #145 Bullet, #146 auth, `feature/alert-redirect` polish). ⚠️ Toujours `export PATH="$HOME/.rbenv/shims:$PATH"` pour `bundle`/`rails`/`rspec` via Bash.
- **Infra prête à déployer** : serveur `78.47.104.157` (Hetzner CX23, Nuremberg, Ubuntu 26.04, **Docker pas encore installé** — Kamal l'installera), SSH root OK via `id_ed25519`, domaine `ninetofine.app` (DNS apex+www → IP, propagé). `Dockerfile` + `config/deploy.yml` générés par Rails 8 (à configurer).
- **▶️ PROCHAINE TÂCHE = déploiement Kamal** (le gros morceau, prévu cet aprem). Méthode : **vérifier la doc Kamal/Rails 8 à jour (WebFetch) AVANT chaque commande** (règle `feedback-verify-external-setup`), expliquer chaque ligne (mode coach). Étapes : configurer `deploy.yml` (registry, host = IP, domaine, secrets `RAILS_MASTER_KEY`) → `kamal setup` (bootstrap Docker serveur + 1er deploy) → SSL Let's Encrypt auto via kamal-proxy sur `ninetofine.app`. ⚠️ **Gérer le build x86 depuis Mac ARM** : configurer un **remote builder** (build sur le serveur) pour éviter l'émulation QEMU lente.
- **Ensuite (ordre roadmap)** : assets emploi (README propre + nettoyage doc public + LinkedIn + CV) → candidatures (programme Le Wagon + sourcing) ; puis **IA suggester** (brainstorm produit dédié) ; React plancher 1-2h/j à réamorcer.

**Reports / backlog** : lancement emploi (⚠️ re-glissé, **prioritaire après déploiement**), skill `job-tracker`, US18 brainstorm produit, US05 delete account, **6 PR Dependabot** ouvertes (#130/131/132/133/141/142 — batch-merge après rebase, NE PAS bump kamal #133 avant 1er déploiement), backlog auth post-deploy (home landing brainstorm, toggle visibilité mot de passe), exos sandbox soir cumulés. _(détail dans `to-do.md` Obsidian)_
