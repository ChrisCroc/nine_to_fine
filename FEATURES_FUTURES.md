...
## Benchmark Alta AI — synthèse & planning (25 juillet 2026)

**Contexte** : benchmark de l'app **Alta Daily (Digital AI Closet, éditeur MWM)**, concurrent direct (garde-robe + stylisme IA). Sources : fiche App Store, test utilisateur (Style Within Grace), analyse technique (DhiWise). Objectif : porter sur Nine to Fine les features à plus fort ROI **au plus vite** et les ordonnancer.

**Réalité vs marketing** (relevé dans le test utilisateur, à ne pas idéaliser) : l'auto-tag d'Alta n'est **pas** parfait (couleur/tissu souvent à corriger, marque/prix manuels) ; le détourage a un mode « Prettify » qui aplatit les pièces en flat-lay (critiqué) ; l'avatar est **bridé** (20 rendus/jour, 8 pièces max) et repose sur de la **génération d'image IA**, pas un mannequin 3D physique ; **pas de drag-to-select** trouvé dans les sources (peut-être un swipe sur version récente).

**⭐ Point technique structurant** : Claude *lit* les images (vision) mais ne les *fabrique pas*. Donc **auto-tag = Claude** (parfait), **détourage + avatar = PAS Claude** (segmentation / diffusion, autres outils). Cette distinction disqualifie tout « yaka un appel Claude » pour l'avatar.

**Les 5 features → décision** (classées par ROI pour l'objectif emploi dev junior Rails + IA) :

1. **Auto-tag catégorie/couleur par vision** — 🟢 meilleur coup, réutilise le pattern IA existant (service + job + tool use + DI), corrige un manque déjà repéré le 20/07 (le suggester ne lit aucune photo, dépend de tags libres épars). **Déjà backloggé** → voir « Taxonomie interne pour le raisonnement de l'IA », **promu sem 31**.
2. **IA tenue météo/événement** — 🟢 déjà ~70 % fait (composer US22). Écart = météo réelle auto-injectée (Open-Meteo gratuit, sans clé). **Déjà backloggé** → voir « API météo auto-injectée », **promu sem 31**.
3. **Détourage photo (fond transparent)** — 🟢 réaliste via API SaaS en job async (PAS Claude) → nouvelle entrée (Priorité haute).
4. **Drag/clic-to-select des pièces** — 🟢 pur front Stimulus, **moins différenciant** → recoupe « Sélection des garments par clic dans l'index » + « Drag-and-drop composition outfit ».
5. **Avatar + try-on** — 🔴 **écarté**. Try-on réaliste = ML spécialisé (diffusion type VITON/IDM-VTON → GPU obligatoire, exclu sur le CX23 CPU ; ou API commerciale coûteuse/boîte noire) → ne démontre **pas** son ingénierie. Alternative légère = **lookbook/collage maison** (pousser la mosaïque d'outfit existante), zéro ML.

**+ Mailers (demande Chris 25/07)** — rendre l'app « pro » via les emails. ⚠️ **Constat sur le code réel** : Devise `:recoverable` est actif (lien « Forgot password » exposé) MAIS aucun SMTP en prod + `default_url_options.host` = `"example.com"` → **le reset de mot de passe échoue silencieusement en prod** (bug de correctness, pas du polish). → nouvelle entrée (Priorité haute).

**⭐ Planning proposé** (validé en partie 25/07 — « au plus vite » en respectant : **plancher React quotidien** [capstone Assembly: Endgame puis **React Advanced / Pro**, créneau apprentissage du midi] + **emploi le week-end**). ⭐ Les features NtF vivent dans le **créneau prod de l'aprem** → elles ne concurrencent pas le créneau React du midi.

- **Week-end / début sem 31** : nav clavier combobox + `kamal deploy` US18 Phase 2 (déjà au plan, dégage la piste).
- **Sem 31 (27 juil–2 août)** — *correctif + sprint qualité IA* (aprem) : **infra mailer** (~0,5 j — répare le reset cassé en prod, décision b du 25/07, à faire tôt) + **#1 auto-tag vision** (1-2 j, tête d'affiche). **#2 météo auto** (0,5-1 j) si place, sinon début sem 32. ⚠️ Semaine chargée — si ça déborde, la météo glisse.
- **Sem 32 (3–9 août)** — *sprint app pro* : reste du **Mailer V1** (`:confirmable` + email de bienvenue, ~0,5-1 j) + **#3 détourage** (0,5-1 j) + météo si reportée.
- **Sem 33 (10–16 août, dernière sem Phase 2)** — *sprint front* : **#4 drag/clic-to-select** (2-3 j) ; peut glisser en Phase 3 (maintenance NtF).
- **Phase 3+ (sem 34+)** : mailers de notification (abonné / commentaire / like = couche engagement) + reliquat drag-select.

**✅ Décision prise (25/07)** : infra mailer **tirée en sem 31** (le reset ne doit pas rester cassé sur l'app vitrine). La couche pro du mailer (confirmable + bienvenue) reste sem 32.

**⚠️ Tension planning à surveiller (React Advanced)** : après les 2 capstones, **React Advanced / Pro** est un cours **substantiel** (multi-semaines) qui occupe le créneau du midi sem 31→33 et **débordera probablement dans la Phase 3** (prévue TypeScript, sem 34+). À arbitrer début Phase 3 : finir React Advanced d'abord, ou l'entrelacer avec le démarrage TypeScript. Rappel stratégique : la cible reste **dev junior Rails** (React = « en apprentissage » au CV, pas survendu) → React Advanced est bon pour la compétence, mais ne doit pas cannibaliser Rails + shipping NtF + candidatures (le vrai livrable borné fin sept).
...
- **Estimation** : 2-3 j.
- **⭐ Benchmark Alta (25/07)** : recoupe le « drag/clic-to-select » observé par Chris (feature #4) — pur front, **moins différenciant** que l'IA pour un poste dev. Recoupe aussi « Drag-and-drop composition outfit » ci-dessus (à fusionner à l'implémentation). L'index filtrable existe déjà (`GarmentFilter`) → le prérequis est levé.
- **Slot suggéré** : **sem 33** (dernière sem Phase 2) ou glisse en Phase 3 (maintenance NtF) — voir planning en tête de fichier *(était : post-emploi oct 2026+)*.
...
- **Contexte d'origine** : samedi 25 juillet 2026 (sem 30), benchmark Alta AI (voir section « Benchmark Alta AI » en tête de fichier, feature #3). Chez Alta, toutes les photos de vêtements sont détourées (fond retiré, on ne voit que la pièce) → catalogue visuellement cohérent. Nine to Fine stocke les photos brutes (décor compris).
- **Description** : à l'upload d'une photo de garment, retirer automatiquement le fond → image à fond transparent, pour une garde-robe propre et homogène (et une future planche/lookbook lisible). ⭐ **PAS Claude** (il ne fabrique pas d'image) : c'est de la **segmentation d'image**.
- **Stack** : 3 options avec trade-offs.
  - **API SaaS** (remove.bg, Photoroom, ou Cloudinary AI background removal) : le plus simple, quelques heures. Payant à l'image + dépendance externe.
  - **Modèle open-source auto-hébergé** (`rembg`, U²-Net) : gratuit mais = **service Python** à faire tourner à côté du Rails sur le CX23 → surcoût infra/maintenance.
  - Ne rien détourer, cadrer l'UX autour de photos déjà propres.
- **Archi recommandée** : API SaaS en **job async** (`RemoveBackgroundJob` Solid Queue → envoie le blob → récupère le PNG transparent → ré-attache en variante Active Storage). Réutilise le pattern job async + Active Storage déjà en place. **Garder l'original** (fallback si le détourage rate + réversibilité).
- **Estimation** : ~0,5-1 j (intégration API + job + variante + UI état « en traitement » + fallback).
- **Slot suggéré** : **sem 32** (sprint app pro, avec les mailers) — voir planning en tête de fichier.

### Configuration des mailers + emails transactionnels (app « pro »)

- **Contexte d'origine** : samedi 25 juillet 2026 (sem 30), demande explicite de Chris (« configurer les mailers pour rendre l'app plus pro »), en marge du benchmark Alta. Recoupe la lecture Odin chap. 38 Mailers (sem 24, note [[mailers-fundamentals]]), dont le setup avait été **volontairement reporté** (YAGNI, « au 1er mailer réel »).
- **⚠️ Bug de correctness constaté sur le code réel (pas juste du polish)** : le model `User` active Devise `:recoverable` → le lien « Forgot your password? » est **exposé dans l'UI** (pages auth habillées, PR #146). MAIS en prod (`config/environments/production.rb`) : `action_mailer.smtp_settings` est **commenté** (aucun SMTP), `delivery_method` non défini, et `action_mailer.default_url_options` vaut encore **`{ host: "example.com" }`** (placeholder). → **le reset de mot de passe échoue silencieusement en prod** (l'utilisateur ne reçoit rien). À corriger.
- **Description — 2 couches** :
  1. **Infra / correctness (rendre le reset fonctionnel)** : choisir un **provider transactionnel** (Postmark, Resend, Brevo, Mailgun, SendGrid, ou Amazon SES) → SMTP creds dans `credentials.yml.enc` + secrets Kamal → `default_url_options host: "ninetofine.app"` + `delivery_method :smtp` + `smtp_settings` → vérif domaine (SPF/DKIM sur le DNS Porkbun) → tester un reset réel en prod. **~0,5 j** (+ propagation DNS).
  2. **Couche pro** : `:confirmable` à l'inscription (email de confirmation) + **email de bienvenue** (`WelcomeMailer`, `Mailer.with(user:).welcome.deliver_later`, ⭐ `after_commit on: :create` sinon job orphelin si rollback — cf. [[callbacks]] / [[mailers-fundamentals]]). **~0,5-1 j**.
- **Backlog séparé — mailers de notification (couche engagement, Phase 3+)** : « X vous suit », « nouveau commentaire sur votre tenue publique », « nouveau like ». Réutilise `deliver_later` + GlobalID + `after_create_commit`. ~1 j+ (par type). Pas dans le Mailer V1.
- **Stack** : provider transac + Action Mailer prod config + Devise mailer views (générables) + éventuel `:confirmable` (migration `confirmation_token`/`confirmed_at`/`confirmation_sent_at` + backfill users existants `confirmed_at: Time.current`) + `WelcomeMailer` + Mailer Preview + tests mailer (RSpec `have_enqueued_mail`).
- **Estimation** : **Mailer V1** (infra + reset + bienvenue + confirmable) ~1-1,5 j ; notifications ~1 j+ (backlog).
- **Slot suggéré** : ⭐ **infra = sem 31** (décidé 25/07 — répare le reset cassé) ; **couche pro (confirmable + bienvenue) = sem 32**. Notifications = Phase 3+. Voir planning en tête de fichier.
...
- **API météo auto-injectée** — géoloc navigateur + service météo tiers → météo réelle injectée dans le prompt (vs l'user qui la tape en texte libre en V1). Résout « été à 13°C ≠ habillé été ». Stack : Geolocation API (ou localisation user en base) + API météo — ⭐ **Open-Meteo recommandé (gratuit, sans clé API** ; ⚠️ licence usage **non commercial**, OK pour la vitrine) vs OpenWeatherMap (clé + quota) — + cache + gestion permission/erreur. Estimation : ~0,5-1 j. Slot : ⭐ **PROMU sem 31** (benchmark Alta 25/07 — feature #2, tack-on du sprint qualité IA ; voir planning en tête de fichier).
